...
def setup_all_matchers(*, deps: MatcherSetupDeps) -> Dict[str, Any]:
    ...
    async def _analyze_group_style(group_id: str) -> Any:
        ...
        try:
            style_api_key = str(
                getattr(deps.plugin_config, "personification_style_api_key", "") or ""
            ).strip()
            if style_api_key:
                provider_api_type = str(
                    getattr(deps.plugin_config, "personification_style_api_type", "")
                    or getattr(deps.plugin_config, "personification_api_type", "openai")
                ).strip().lower()
            else:
