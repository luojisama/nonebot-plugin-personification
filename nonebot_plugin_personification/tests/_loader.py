...
_PACKAGE_NAME = "nonebot_plugin_personification"
...
def load_personification_module(module_name: str):
    ...
    if not parts or parts[0] != _PACKAGE_NAME:
        raise ValueError(f"unsupported module namespace: {module_name}")

    _ensure_namespace_package(_PACKAGE_NAME, _PERSONIFICATION_DIR)
    ...
    file_path = _PERSONIFICATION_DIR.joinpath(*parts[1:]).with_suffix(".py")
