...
from ..core.sticker_cache import get_sticker_files
from .vision_caller import VisionCaller
...
def _score_sticker(meta: dict, mood: str, context: str, proactive: bool) -> int:
    haystack = " ".join(
        [
            ...
            " ".join(meta.get("scene_tags", []) or []),
        ]
    ).lower()
    score = 0
    for token in [mood, context]:
...
def select_sticker(
    ...
    *,
    ...
    for sticker in stickers:
        ...
        if score >= int(config.get("semantic_threshold", 1)):
