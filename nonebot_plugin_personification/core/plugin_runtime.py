...
def _build_sign_in_fallbacks() -> tuple[bool, Any, Any, Any, Any]:
    try:
        from nonebot_plugin_shiro_signin.utils import get_user_data, load_data, update_user_data  # type: ignore
        from nonebot_plugin_shiro_signin.config import get_level_name  # type: ignore
...
def _build_qzone_fallbacks() -> tuple[bool, Any, Any]:
...
class PluginRuntimeBundle:
    ...
    def make_job_setup_deps(self, *, check_proactive_messaging: Any) -> JobSetupDeps:
        return JobSetupDeps(
            sign_in_available=self.sign_in_available,
            load_data=self.load_data,
            get_now=get_beijing_time,
...
def build_plugin_runtime(
    *,
    ...
    qzone_publish_available, publish_qzone_shuo, update_qzone_cookie = _build_qzone_fallbacks()
