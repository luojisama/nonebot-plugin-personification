...
- **只读社交平台 MCP**：在 B站、抖音、贴吧和小黑盒中并发检索与读取；默认按全局 10 条上限公平聚合，保留独立来源组、覆盖状态、规范 URL 和可审计证据交付，并将小黑盒作者正文与文章图片作为受限的不可信多模态证据交给 Agent
...
- 小黑盒搜索选中帖子后并发读取作者文章容器，向结果补充完整正文、`image_refs`、图片数量和详情状态；详情页缺少互动计数时不再把已选来源静默过滤掉。
- Agent 最多接收 4 张小黑盒文章图片；图片需通过平台域名白名单、公共地址解析、跳转复核、4 MiB 下载上限、像素上限和 JPEG 重编码，正文与图片提示均继续标记为不可信数据。
- WebUI 原生 MCP 预览增加小黑盒图集和图片计数；搜索与详情缓存升级，避免旧的封面-only 数据复用。
- 同步源码主线至 `4f63b48`，源码完整测试 `1989 passed`。

### 0.7.1

- 修复小黑盒公开搜索页动态内容尚未挂载时被误判为零候选的问题，检索会在平台超时预算内等待真实帖子链接。
- 修复同一平台并发详情读取复用单个浏览器页导致导航互相覆盖的问题；每个只读操作使用独立页面并在完成后释放。
- 升级社交搜索缓存算法版本，避免旧的零候选结果继续污染多源聚合。

### 0.7.0

- **同步源码主线至 `64420ca`**：带回 0.6.2 之后的 QQ/QZone 出站账本、用户策略、远程 Skill 与托管 MCP、数据迁移、群成员画像、Provider 兼容诊断及 WebUI 运维增量，同时保留发布版入口、localstore/htmlrender 和桥接文件适配。
- **新增原生只读社交研究 MCP**：隔离 B站、抖音、贴吧和小黑盒登录 profile，提供 `social_content_search`、`social_content_read`、`research_game_slang` 三个只读工具；默认搜索全部启用平台，结果总上限为 10。
- **修复小黑盒和多源聚合**：使用公开 `/app/bbs/home`、`/app/search/list` 与 `/app/bbs/link/<数字ID>` 路由，正文限定公开文章 DOM；增加公平平台覆盖、来源分组、详情失败降级、缓存版本和机器可读覆盖状态。
- **贯通证据、Trace 与 QQ 投递**：社交研究回复保证规范来源链接；工具执行、证据交付和协议发送分开记录，安全拦截可恢复为结构化证据尾注，缺链接或发送失败不再误报完整成功。
- **校准群聊参与边界**：仍由 LLM 结合最近话题、参与者和消息目标决策；不再仅因没有 @、随机插话或目标暂不明确而机械静默，明确他人点对点、纯媒体/回声、私密话题和上下文不足仍优先沉默。
- **更新发布依赖与验证基线**：补齐 Playwright、aiosqlite、ruamel.yaml、qrcode、Alconna/Uninfo 等当前运行依赖，并同步完整测试、语义扫描和 WebUI 静态资源。
